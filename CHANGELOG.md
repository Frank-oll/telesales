...
- **从旧版(≤1.2.0)升级后首次打开不弹「本次更新」**:`maybeShowWhatsNew` 原先把"无 `seen` 标记"一律当作全新安装,导致首次引入该功能时所有升级用户都不弹。改为用"本地是否已有业务数据(历史会话 / 意向客户 / 通话记录)"兜底识别——老用户升级会正常展示「本次更新」,只有真正一条数据都没有的全新安装才不弹。

### 优化

- **dmg 安装包内置「解锁助手 + 安装说明」 + 黑板风手绘引导背景,降低首次安装门槛**:
  - dmg 现包含四样:`Telesales.app`、`Applications` 软链接、`双击解锁并打开.command`(双击 / 右键打开即自动移除隔离标记并启动 App,优先免密码、必要时再回退 sudo)、`安装说明（必看）.txt`(2 步图文)。
  - dmg 背景图改为**黑板粉笔风手绘引导**(`electron/build/gen-bg.py` 由 Pillow 生成):虚线框把四项分成两区,配「先把这个拖进去」「再双击运行这个」手写标注 + 手绘箭头,粉丝一眼就懂怎么装。这套排布作为后续版本的标准沿用。
  - 未签名版本的粉丝不必再手敲终端命令。
