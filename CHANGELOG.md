...
- **dmg 安装包内置「解锁助手 + 安装说明」,降低首次安装门槛**:dmg 现包含四样——`Telesales.app`、`Applications` 软链接、`双击解锁并打开.command`(双击 / 右键打开即自动移除隔离标记并启动 App,优先免密码、必要时再回退 sudo)、`安装说明（必看）.txt`(2 步图文);并调整 dmg 窗口尺寸 / 图标布局让四项排布更协调。未签名版本的粉丝不必再手敲终端命令。
